...
Use the SQLToolsAndLibraries project in the msdata ADO organization with the azure-devops-sqltools MCP server.
...
| Diagnose PR test failure | **VS Code Chat:** use the `@diagnose-test-failures` agent with a PR or build link (auto-detects from current branch). **SweAgent / terminal-only:** use the `diagnose-test-failures` skill (`.github/skills/diagnose-test-failures/SKILL.md`) — it downloads `.trx` test result artifacts from the build. Do NOT use REST API or PowerShell for ADO access. |
...
- Update CHANGELOG.md under the "Unreleased" section for any externally visible changes (new features, bug fixes, breaking changes).
...
- Tests failing only in CI: Check for dependency on local environment (tenant login missing). Add explicit skip or setup logic. For systematic diagnosis of PR build failures, follow the procedure in `.github/agents/diagnose-test-failures.md`.
