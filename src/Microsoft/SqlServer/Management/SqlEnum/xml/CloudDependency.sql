...
-- These type-to-int mappings correspond to TypeToNo values in SqlEnumDependencies.cs
set @u = 3
set @udf = 0
set @v = 2
set @sp = 4
...
begin
    ...
    begin
      ...
      set @rows = @rows + @@rowcount
        ...
        -- non-schema bound security policies are excluded from sql_expression_policies
        insert #t1 (object_id, object_name, object_schema, object_db, object_type, relative_id, relative_name, relative_schema, relative_db, relative_type, schema_bound, rank)
          select distinct pol.object_id, pol.name, SCHEMA_NAME(pol.schema_id), t.object_db, 19, t.object_id, t.object_name, t.object_schema, t.object_db, t.object_type, 0, 2
          from #t1 as t
          join sys.security_predicates p on p.target_object_id = t.object_id join sys.security_policies pol on pol.object_id = p.object_id and pol.is_schema_bound = 0
        set @rows = @rows + @@rowcount
      ...
      --trigger references table, procedure
        ...
        select  case when obj.type in ('C' , 'D') then obj.parent_object_id else obj.object_id end,case when obj.type in ('C','D' ) then OBJECT_NAME(obj.parent_object_id) else OBJECT_NAME(obj.object_id) end,schema_name(obj.schema_id),t.object_db,
               ...
               when obj.type in ('SO') then @sequence
               when obj.type in ('SP') then @secpol
               end,  t.object_id, t.object_name,t.object_schema,t.object_db, t.object_type, dp.is_schema_bound_reference,@iter_no + 1
         from #t1 as t
         join sys.sql_expression_dependencies as dp on 
         (dp.referenced_id = t.object_id Or (dp.referenced_entity_name = t.object_name collate database_default AND dp.is_caller_dependent = 1 ))
         join sys.objects as obj on obj.object_id = dp.referencing_id and obj.type in ( 'U', 'V', 'P', 'RF', 'PC', 'TR', 'TF', 'FN', 'IF', 'FS', 'FT', 'C' , 'D', 'SO', 'SP')
         --join sys.objects as obj on obj.object_id = dp.referencing_id and obj.type in ( 'U', 'V', 'P', 'RF', 'PC', 'TR', 'TF', 'FN', 'IF', 'FS', 'FT', 'C' , 'D', 'EC', 'SO', 'SP')
         where @iter_no = t.rank
      set @rows = @rows + @@rowcount
    ...
    begin
        ...
        BEGIN
            ...
            SET @query = 'update #t2 set object_db = N' + quotename(@dbname, '''') + ', object_id = obj.object_id, object_type = 
                            ...
                            ' when obj.type = ''SP'' then ' + CAST(@secpol as nvarchar(8)) +
...
--final select to assign parent objects to triggers
