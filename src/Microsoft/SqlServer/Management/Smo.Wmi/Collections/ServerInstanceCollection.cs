...
namespace Microsoft.SqlServer.Management.Smo.Wmi
{
	...
	///</summary>
	public sealed class ServerInstanceCollection : WmiCollectionBase, ICollection 
	{

		internal ServerInstanceCollection(WmiSmoObject parentInstance)  : base(parentInstance)
		{
		}
		...
		public ServerInstance this[Int32 index]
		{
			get
			{ 
		...
		internal ServerInstance GetObjectByIndex(Int32 index)
		{
			if(!initialized)
				InitializeChildCollection();
			
			return innerColl.GetByIndex(index) as ServerInstance;
		}
		
		// returns wrapper class
		public ServerInstance this[string name]
		{
			get
			{
		...
		internal ServerInstance GetObjectByName(string name)
		{
			...
			return instanceObject as ServerInstance;
		}
		...
		internal object InitializeChildObject(string name)
		{
			...
			ServerInstance childobj = new ServerInstance(this, name);
		...
		internal void InitializeChildCollection()
		{
			...
			ParentInstance.EnumChildren(typeof(ServerInstance).ToString(), this);

			// now merge the old collection into the new one
			foreach(DictionaryEntry de in oldColl)
			{
				ServerInstance objMAPPED_TYPE = de.Value as ServerInstance;
		...
		public void CopyTo(ServerInstance[] array, Int32 index)
		{
		...
		void ICollection.CopyTo(Array array, Int32 index)
		{
			...
			{
				array.SetValue( (ServerInstance)de.Value, idx++);
			}
		}
		...
		public IEnumerator  GetEnumerator() 
		{
			if(!initialized)
				InitializeChildCollection();
			return new ServerInstanceCollectionEnumerator(this);
		}
		...
		internal sealed class ServerInstanceCollectionEnumerator : IEnumerator 
		{
			internal IDictionaryEnumerator baseEnumerator;
			
			internal ServerInstanceCollectionEnumerator(WmiCollectionBase col) 
			{
			...
			{ 
				get 
				{	
					return baseEnumerator.Value as ServerInstance;
				} 
			}
			
			public ServerInstance Current 
			{ 
				get 
				{	
					return baseEnumerator.Value as ServerInstance;
