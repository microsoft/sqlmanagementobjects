...
namespace Microsoft.SqlServer.Management.Sdk.Sfc
{
    ...
    // SFC has a duplicated copy of class PostProcess, inherited from SMO
    // Since it's not used anywhere, remove it from build.
    // Leaving the code in place in case we ever decide to refactor SMO and make it use SFC's PostProcess
	...
	public abstract class PostProcess
	{
		...
		internal void CheckDataReaderSupport()
		{
			if( !this.SupportDataReader )
			{
				...
				throw new QueryNotSupportedEnumeratorException(SfcStrings.QueryNotSupportedPostProcess(props));
			}
		}
		...
		protected int BinarySearch(DataRowCollection col, string objectIdentifier, string columnName)
		{
			...
			{
				int mid = ( start + end ) / 2;
				int cmp = objectIdentifier.CompareTo(col[mid][columnName]);
				if( -1 == cmp )
				{
					end = mid - 1;
				}
		...
		/// <summary>
		///generate a 'use' statement for the database name</summary>
		protected String GetUse(String databaseName)
		{
			return String.Format(CultureInfo.InvariantCulture, "use [{0}]\n", Util.EscapeString(databaseName, ']'));
		}
